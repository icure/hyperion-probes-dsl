...
data class HistogramBucket(
    ...
    fun toHistogramSnapshot(percentiles: Iterable<Double>? = null, summaryOutput: ((PrintStream, Double) -> Unit)? = null): HistogramSnapshot =
        HistogramSnapshot(
            ...
            sum,
            max,
            percentiles?.map { ValueAtPercentile(it, histogram.getQuantile(it)) }?.toTypedArray(),
            histogram.nonEmptyBinsAscending().map { CountAtBucket(it.binIndex.toDouble(), it.binCount.toDouble()) }.toTypedArray(),
            summaryOutput
        )
