...
fun ExtractorFactory.over(duration: Duration, clock: Clock) = ExtractorFactoryParams(this) {
	TimeWindowCollector(duration, Duration.ofSeconds(60), clock)
}

fun SingleExtractorFactory.over(duration: Duration, clock: Clock) = ExtractorParams(this.getExtractor("value")) {
	TimeWindowCollector(duration, Duration.ofSeconds(60), clock)
}